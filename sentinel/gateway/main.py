"""
SENTINEL Gateway — FastAPI Application v4.0
=============================================
...
# ── Constants ─────────────────────────────────────────────────────────────────
AGENT_COUNT = 19       # v4: 7 (v1) + 5 (v2) + 3 (v3) + 4 (v4 new agents)
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    logger.info("🛡️  SENTINEL gateway v4.0 starting up …")
...
app = FastAPI(
    ...
    version="4.0.0",
    description="Production-grade 19-agent security middleware for enterprise LLM deployments",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
)
...
async def readiness(db: AsyncSession = Depends(get_db)):
    ...
    checks["agents"] = len(_agents) == AGENT_COUNT   # v3: 15 agents
